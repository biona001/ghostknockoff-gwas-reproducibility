...
s = rep(1,p)
Sigma_0 = diag(2*p)
Sigma_0_inv = diag(2*p)
chol_Sigma_0 = diag(2*p)
...
simulation_1 <- function(amplitude){
  ...
  # GK-susie-rss
  susie_fit = susie_rss(z = rbind(Z,Z_tilde)/sqrt(n), R=Sigma_0, n=n,var_y=var(y))
  coef = susie_fit$pip
  W_susie = abs(coef[1:p]) - abs(coef[(p+1):(2*p)])
...
simulation <- function(amplitude,rep=200){
  result = sapply(rep(amplitude,rep),simulation_1)
  return(c(apply(result,1,mean), apply(result,1,sd)/sqrt(rep)))
}

result = simulation(amp)
print(result)