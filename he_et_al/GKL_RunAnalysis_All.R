...
# for testing
replicate = 11
zscore.file = "/scratch/users/bbchu/ghostknockoff-gwas-reproducibility/data/AD_Zscores_Meta.txt"
knockoff.dir = "/scratch/users/bbchu/ghostknockoff-gwas-reproducibility/data/EUR"
SummaryStat.info.file = "/scratch/users/bbchu/ghostknockoff-gwas-reproducibility/data/SummaryStatInfo.txt"
cS2G.file = "/scratch/users/bbchu/ghostknockoff-gwas-reproducibility/data/topcS2GGene_allVariants.csv"
out.dir = "/scratch/users/bbchu/ghostknockoff-gwas-reproducibility/Results"
...
for(initial.chr in 19){  
  ...
  KF.SLD.names<-list.files(KF.dir,pattern = 'LD')
  KF.info.names<-paste0('Info',sub('.h5','',sub('LD','',KF.SLD.names)),'.csv')
  ...
  for(set in 1:N_set){
    ...
    D<-h5read(file.path(KF.dir,KF.SLD.names[set]),'D')
    S<-h5read(file.path(KF.dir,KF.SLD.names[set]),'S')
    Sigma<-h5read(file.path(KF.dir,KF.SLD.names[set]),'Sigma')
    group_reps<-h5read(file.path(KF.dir,KF.SLD.names[set]),'group_reps')
    SigmaInv<-h5read(file.path(KF.dir,KF.SLD.names[set]),'SigmaInv')
    groups<-h5read(file.path(KF.dir,KF.SLD.names[set]),'groups')
    #H5Fclose(KF.data)
    ...
    KF.Z<-temp.Zscores[match.index,'Z']
    ...
    print(paste0('chr',initial.chr,' set ', set, ', f = ', KF.info.names[set], ', max beta = ', temp.beta[which.max(abs(temp.beta))], ' shrinkage = ', shrinkage.lambda))
